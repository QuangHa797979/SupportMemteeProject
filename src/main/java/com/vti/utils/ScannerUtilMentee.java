...
public class ScannerUtilMentee {
	...
	public static String readProjectName(Scanner scanner, String errorMessage) {
		while (true) {
			try {
				String result = scanner.nextLine().trim();
				if (result != null & !result.isEmpty()) {
					if (result.matches(PATTERN_STATUS_1) || result.matches(PATTERN_STATUS_2)
							|| result.matches(PATTERN_STATUS_3) || result.matches(PATTERN_STATUS_4)) {
						return result;
					} else {
						System.out.println("Status must be send or pending or approve or refuse");
					}
				}
			} catch (Exception e) {
				showError(errorMessage);
	...
	public static String readUsername(Scanner scanner, String errorMessage) {
		while (true) {
			try {
				String result = scanner.nextLine().trim();
				if (result != null & !result.isEmpty()) {
					if (result.matches(PATTERN_USERNAME)) {
						return result;
					} else {
						System.out.println("Username is not true pattern! Try again");
					}
				}
			} catch (Exception e) {
